...
export default () => {
  ...
  const show = useCallback(() => {
    try {
      sdk.show();
      // TODO(ak): To remove once Ramp handle dark-mode containers on their side
      if (sdk?.domNodes?.shadowHost?.style) {
        sdk.domNodes.shadowHost.style.colorScheme = 'only light';
      }
    } catch (e: any) {
      if (
