...
` as TypedDocumentNode<CreateWithdrawMutation, CreateWithdrawMutationVariables>;

export default () => {
  const [create] = useMutation(CREATE_WITHDRAW_MUTATION);
  ...
  return async (amount: string, to: string) => {
    const deal = generateDeal({
      ...
      receiver: {
        __typename: 'User',
        slug: '',
        viccAddress: sponsorAccountAddress,
      },
    });
