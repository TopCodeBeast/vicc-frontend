...
import usePrepareFastWithdrawal from '@core/hooks/withdrawals/usePrepareFastWithdrawal';
import { Currency } from '@core/lib/currency';
import { isAddress } from '@core/lib/ethereum';
import { glossary } from '@core/lib/glossary';
import { RoundingMode, fromWei, roundCeilFloat, toWei } from '@core/lib/wei';
...
export const WithdrawEthTo = () => {
  ...
  const [prepareIsLoading, setPrepareIsLoading] = useState(false);
  ...
  const feeAmount = stopSponsorGasFees
    ? fromWei(unquantizeAmount(transfer?.feeInfoUser?.feeLimit || '0'))
    : 0;

  useEffect(() => {
    const timer = setTimeout(() => {
      const disableDueToEthAmount = maxEthAmount < amountToWithdraw + feeAmount;
      setFormErrors({
        amountToWithdraw:
          disableDueToEthAmount || maxFiatAmout < fiatAmountToWithdraw,
        to: to !== '' && !isAddress(to),
      });
    }, 1000);
    return () => {
  ...
  useEffect(() => {
    async function prepareWithdraw() {
      if (to && to !== '' && amountToWithdraw && isAddress(to)) {
        setPrepareIsLoading(true);
        setTransfer(null);
        const prepareFastWithdrawal = await prepare(
          amountToWithdraw.toString(),
          to
        );
        if (prepareFastWithdrawal.fastWithdrawal) {
          setTransfer(prepareFastWithdrawal.fastWithdrawal);
        }
        setPrepareIsLoading(false);
      }
    }
    prepareWithdraw();
  }, [prepare, to, amountToWithdraw]);
  ...
  const submitIsDisabled =
    ...
    !amountToWithdraw ||
    to === '' ||
    (stopSponsorGasFees && prepareIsLoading);
        ...
        {stopSponsorGasFees && (
          <div>
            <WithdrawEthToFeesAndTotal
              weiAmountToWithdraw={toWei(amountToWithdraw)}
