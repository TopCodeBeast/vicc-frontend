...
import { BundledTokenAuctionName } from '@core/components/bundled/BundledTokenAuctionName';
...
export const AuctionNotification = ({ notification, ...rest }: Props) => {
  ...
  const { name, createdAt, tokenAuction, sport, read } = notification;
  ...
  const content =
    tokenAuction.nfts.length === 1 ? (
      ...
      <BundledTokenAuctionName tokenAuction={tokenAuction} Variant={Text16} />
    );
