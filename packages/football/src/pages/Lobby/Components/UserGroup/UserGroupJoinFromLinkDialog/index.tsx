...
import useJoinSo5UserGroup from '@football/pages/Lobby/Components/UserGroup/CreateOrJoin/useJoinSo5UserGroup';
import UserGroupPreview from '@football/pages/Lobby/Components/UserGroup/UserGroupDialog/Steps/Congrats/UserGroupPreview';
import useGetUserGroupByJoinSecret from '@football/pages/Lobby/Components/UserGroup/useGetUserGroupByJoinSecret';
...
const UserGroupJoinFromLinkDialog = ({ joinSecret }: Props) => {
  const navigate = useNavigate();
  const joinSo5UserGroup = useJoinSo5UserGroup();
  const { data } = useGetUserGroupByJoinSecret(joinSecret);
  ...
  const userGroupJoinState = getUserGroupJoinState();

  const navigateToUserGroupDetails = () => {
    navigate(
  ...
  const onJoin = async () => {
    const { data: joinData } = await joinSo5UserGroup({ joinSecret });
  ...
  const renderData = {
    ...
    [joinStates.alreadyJoined]: {
      onClick: navigateToUserGroupDetails,
      title: (
        <FormattedMessage
  ...
  return (
    <DialogContainer>
      ...
      />
      <DialogContent>
        <Title3>{renderData[userGroupJoinState].title}</Title3>
        <Subtitle>{renderData[userGroupJoinState].subtitle}</Subtitle>

        <JoinButton
          onClick={renderData[userGroupJoinState].onClick}
          medium
          color={renderData[userGroupJoinState].buttonColor}
        >
          <ButtonTextWrapper>
            {renderData[userGroupJoinState].buttonText}
          </ButtonTextWrapper>
        </JoinButton>
      </DialogContent>
    </DialogContainer>
...
export default UserGroupJoinFromLinkDialog;
