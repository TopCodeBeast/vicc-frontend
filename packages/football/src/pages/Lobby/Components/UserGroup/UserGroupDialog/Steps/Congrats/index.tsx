...
import { GetUserGroupQuery } from './__generated__/index.graphql';

export const GET_USERGROUP_QUERY = gql`
  query GetUserGroupQuery($slug: String!) {
    football {
      so5 {
...
export const Congrats = ({ slug, redirectToManagerHome }: Props) => {
  ...
  const { data } = useQuery<GetUserGroupQuery>(GET_USERGROUP_QUERY, {
    variables: { slug: userGroupSlug },
  });
    ...
    <DialogContent>
      <Content>
        ...
        {redirectToManagerHome ? (
          ...
          <>
            <SocialShare
              url={generateUserGroupInviteLink(
                joinSecret,
                currentUser?.slug,
                so5LeaderboardType
              )}
