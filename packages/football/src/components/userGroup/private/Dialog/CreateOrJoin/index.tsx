...
import { alreadyJoinedUserGroup } from '@football/components/userGroup/private/helper';

import { JoinPrivateUserGroupMutation } from './__generated__/useJoinPrivateUserGroup.graphql';
import useJoinPrivateUserGroup from './useJoinPrivateUserGroup';
...
const CreateOrJoin = ({ next }: Props) => {
  ...
  const joinPrivateUserGroup = useJoinPrivateUserGroup(true);
  const { data, loading } = useQuery(
    ELIGIBLE_TOURNAMENTS_FOR_PRIVATE_USER_GROUP_QUERY
  );
  ...
  const onJoin = async (
    ...
    if (joinSecret) {
      const submitData = await joinPrivateUserGroup({ joinSecret });
      onResult(submitData);
    } else {
      onCancel();
    }
      ...
      <InviteBlock>
        {loading ? (
          ...
          <>
            ...
            {!data?.vicc5Root.eligibleTournamentsForVicc5UserGroups
              .length ? (
              <Text16>
                <FormattedMessage
